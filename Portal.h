...
const char HTML_DEV_PANE_TPL[] PROGMEM = "<div class=\"tab-pane\" id=\"device_{dev_name}\"><input type=\"text\" name=\"device_name\" value=\"{dev_name}\"><input name=\"device_type\" value=\"{dev_type}\"><p>Elenco tasti definiti:</p></div>";
...
//const char JAVASCRIPT_EDIT_DEVICE[] PROGMEM ="document.addEventListener('DOMContentLoaded',function(event){$('#frm_edit_dev_name').on('input',function(){var a=$('#form_selected_device_name').val().trim(),b=$(this).val().trim();if(!a||b!=a){var c=!1;$('#device-template li a').each(function(a){return c=$(this).text()==b,!c}),this.setCustomValidity(c?'Il nome del dispositivo è già in uso':'')}});$(\"#device-name-edit\").on(\"shown.bs.modal\",function(){var a=$(e.relatedTarget).data(\"action\");if(\"new-device\"==a)$(\"#frm_edit_dev_name\").val(\"\"),$(\"#frm_edit_curr_dev_name\").val(\"\");else if(\"edit-device\"==a){var i=$(\"#devices-list\").find(\"li.active a\").attr(\"href\"),d=$(i).children(\"input[name=device_name]\").val(),v=$(i).children(\"input[name=device_type]\").val();$(\"#frm_edit_dev_name\").val(d),$(\"#frm_edit_curr_dev_name\").val(d),$(\"#frm_edit_dev_type\").val(v)}});});";
...
"document.addEventListener(\"DOMContentLoaded\",function(e){$(\"#device-edit\").on(\"shown.bs.modal\",function(e){var i=$(e.relatedTarget).data(\"action\");if(\"new-device\"==i)$(\"#frm_edit_dev_name\").val(\"\"),$(\"#frm_edit_curr_dev_name\").val(\"\");else if(\"edit-device\"==i){var t=$(\"#devices-list\").find(\"li.active a\").attr(\"href\"),d=$(t).children(\"input[name=device_name]\").eq(0).val(),a=$(t).children(\"input[name=device_type]\").eq(0).val();$(\"#frm_edit_dev_name\").val(d),$(\"#frm_edit_curr_dev_name\").val(d),$(\"#frm_edit_dev_type\").val(a)}}),$(\"#device-edit\").on(\"hidden.bs.modal\",function(){$(\"#frm_edit_dev_name\")[0].setCustomValidity(\"\"),$(this).data(\"bs.modal\",null)}),$(\"#frm_edit_dev_name\").on(\"input\",function(){var e=$(\"#frm_edit_curr_dev_name\").val().trim(),i=$(this).val().trim();if(!e||i!=e){var t=!1;$(\"#devices-list li a\").each(function(e){return t=$(this).text()==i,!t}),this.setCustomValidity(t?\"Il nome del dispositivo è già in uso\":\"\")}});"
"\n"
...
"                if (!dev_a.length) {\n"
"                    dev_tab = $('#device-template').children().eq(0).clone();\n"
